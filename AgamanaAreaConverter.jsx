...
/* ---------- In-memory persistence (artifact-safe) ---------- */
const store = {
  lang: "en", theme: "light", recent: [], installDismissed: false,
};
...
export default function App() {
  ...
  useEffect(() => { store.theme = dark ? "dark" : "light"; }, [dark]);
  useEffect(() => { store.recent = recent; }, [recent]);
    ...
    <div style={{ ...theme, fontFamily: bodyFont, minHeight: "100vh", background: "var(--bg)", color: "var(--ink)", paddingBottom: 78, transition: "background .3s,color .3s" }}>
      ...
      {showInstall && (
        <div className="rise" style={{ position: "fixed", bottom: 86, left: 12, right: 12, maxWidth: 540, margin: "0 auto", background: "var(--card)", border: "1px solid var(--line)", borderRadius: 16, padding: "14px 16px", boxShadow: "0 8px 30px rgba(0,0,0,.18)", zIndex: 40, display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <button onClick={() => { store.installDismissed = true; setShowInstall(false); showToast(t.install); }} style={{ ...solidBtn, padding: "8px 14px", fontSize: 13 }}>Install</button>
          <button onClick={() => { store.installDismissed = true; setShowInstall(false); }} aria-label="Dismiss" style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer" }}><X size={18} /></button>
        </div>
      )}
